...
const Spinner = (props: SpinnerType) => {
	const { type = 'inline', size = 60, is_dim = false } = props;
...
const SpinnerWrap = styled.div<{ type: string; is_dim: boolean }>`
	width: 100%;
	display: flex;
	align-items: center;
	...
	${({ type }) => (type === 'page' ? `position: fixed;` : ``)};
	${({ type }) => (type === 'page' ? `height: 95vh;` : ``)};
	${({ type }) => (type === 'page' ? `top: 0;` : ``)};
	${({ type }) => (type === 'page' ? `left: 0;` : ``)};
	${({ type }) => (type === 'page' ? `padding: 0;` : ``)};
	${({ type }) => (type === 'page' ? `zIndex: 9999;` : ``)};

	${({ is_dim }) => (is_dim ? `background: rgba(0,0,0,0.4);` : ``)};
	${({ is_dim }) => (is_dim ? `height: 100vh;` : ``)};
`;
